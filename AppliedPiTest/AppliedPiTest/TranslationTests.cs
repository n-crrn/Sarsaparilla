...
";
        ...
        Dictionary<Socket, int> c0Write1History = new() { { c1Out, 0 } };
        Dictionary<Socket, int> c0Write2History = new() { { c1Out, 1 } };
        ...
        HashSet<IMutateRule> expectedMutations = new()
        {
            ...
            new OpenSocketsRule(new List<Socket>() { c2In }, new List<Socket>() { c1Out }),
            new ReadRule(c2In, "v"),
            new AttackChannelRule(c2In, "v"),
            ...
            new FiniteWriteRule(cInfOut, new Dictionary<Socket, int>(), lastPremises, new NameMessage("@f@0")),
            new BasicRule(new() { Event.Know(new NameMessage("@f@0"))}, new NameMessage("f")),
            new ReadResetRule(cInfIn),
            new ReadRule(cInfIn, "x"),
            new AttackChannelRule(cInfIn, "x"),
    ...
    /*
     * Test disabled until a better, more predictable identification scheme found for the 
     * deconstruction rule.
    [TestMethod]
...
process !( out(c, s) |  in(c, v: bitstring) ).";
        ...
        HashSet<IMutateRule> expectedMutations = new()
        {
            ...
            new InfiniteCrossLink(
                cOut, 
                cIn, 
                new Dictionary<Socket, int>(), 
                new(), 
                new NameMessage("s")),
...
process 
  ...
  if v = w && v <> x then out(c, v).";
        ...
        {
            // Branch 0 (in(c, v)).           
            new OpenSocketsRule(new List<Socket>() { cIn, cOut }, new List<Socket>()),
            new KnowChannelContentRule(cOut),
            new ReadRule(cIn, "v"),
            new AttackChannelRule(cIn, "v"),
